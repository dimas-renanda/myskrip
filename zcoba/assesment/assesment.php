...
$no=1;
         ...
         // echo '<td>'.$data['id'].'</td>';
          ...
          echo '<td >      <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#myModal'.$data['id'].'"><i class="fa fa-edit"></i></button>';
...
echo '
               ...
               $semester = extractSemesterValue($data['fullname']);
               $pr = extractYearValue($data['fullname']);
               $classname = extractClassNameValue($data['fullname']);
               $whatclass = '('.extractClassValue($data['fullname']).')';
               // var_dump($semester);
                //var_dump($classname);
               //$periode = '2022S1';
               $periode = $pr.'S'.$semester;
   $token = hasher("$periode,$kodeunit,$kodemk");
    $evalurl  = 'https://obe.petra.ac.id/serviceout.php?t=get_asesmen_list&kodemk='.$kodemk."&periode=".$kodeunit."&periode=".$periode.'&kodeunit=15&token='.$token;


    $dataeval = file_get_contents($evalurl);
    //var_dump($homepage);
    $jsonArrayEvalResponse = json_decode($dataeval,true);
foreach ($jsonArrayEvalResponse as $dataev) {
   //echo '<option value="1">'.$kodemk.'</option>';
   echo '<option value="'.$dataev['name'].'">'.$dataev['name'].'</option>';
}
...
echo '
               <div class="modal-footer d-flex justify-content-center">
                  <button id="redit" class="btn btn-default btn-dark btn-block text-uppercase"><i class="fa fa-edit prefix grey-text">  </i> Create Assesment</button>
               </div>
            </form>
              </div>
          </div>
